...
{<ArticleImage src="/article-images/blog/fork.png" type="table" centered={true} alt="A fork" caption="Fig.1.: A fork." />}
...
For instance, the program `PredecessorIsValid` could be a program that *a)* verifies
 ...
 - the correct update of the [mutator set](/articles/mutator-sets), which is the data structure that stores UTXOs
...
At no point does any code anywhere in the code base rely on cryptographic hardness assumptions that are known to be broken in the presence of quantum computers. It follows that Neptune Cash is plausibly quantum-secure [at genesis](/articles/post-quantum-security-at-genesis).
...
An integral mempool is a pair `(peaks, proof)` where `peaks` is the peaks list of an [MMR](/articles/mmr) and where the `proof` establishes that the MMR was only ever updated by adding a valid transaction. With access to such a data structure, a proof of membership establishes transaction validity, and can be used instead of the original transaction proof.
...
[^1]: Most notably, when the writepaper was written, [Mutator Sets](/articles/mutator-sets) had not been invented yet. The whitepaper therefore outlines an inferior solution with the same effect.
