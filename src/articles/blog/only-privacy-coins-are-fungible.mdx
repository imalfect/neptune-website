...
{<ArticleImage src="/article-images/blog/lady-justice-statue.png" type="table" centered={true} alt="Lady Justice statue" caption="Lady Justice recognizes the importance of the fungibility of banknotes" />}
...
{<ArticleImage src="/article-images/blog/hew-crawfurd-presenting-his-evidence-to-the-judge.png" type="table" centered={true} alt="Hew Crawfurd in court" caption="Hew Crawfurd making his case to the courth, as imagined by Midjourney" />}
...
## Neptune
Neptune is built with this in mind: To truly eliminate risks when receiving or holding crypto currencies, the blockchain must be private; the transaction history of each coin must be hidden such that any legal claim for the reversal of a transaction is voided. Previous attempts at building privacy-preserving blockchains have not reached their full potential, we believe, [because they are not scalable](/articles/scalable-privacy-problem/). Modern cryptography can solve this shortcoming. In particular, the combination of a [zero-knowledge STARK virtual machine](/articles/announcing-tvm) and [mutator sets](/articles/alphanet) will make it impossible to link inputs and outputs in a transaction and it will do so without demanding an unreasonable computational workload from the network participants.
